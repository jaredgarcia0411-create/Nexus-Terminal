...
- **D8. Smoke runbook is markdown only.** The deploy smoke checklist (AEV2-509) ships as `docs/ops/agents-deploy-smoke.md` — not as a script. Codex does NOT write a smoke runner. The user walks each item by hand because most steps require live Discord interaction. The file mirrors the full External Validation Checklist below — operator-owned pre-smoke prerequisites, the live smoke sequence, and post-smoke closeout items — in the same order with the same expected results. `docs/ops/agents-launch-validation.md` remains the canonical deep-dive for env/config parity, while `docs/ops/agents-deploy-smoke.md` is the canonical launch checklist. Sprint 4's "completion" of AEV2-509 means the markdown exists and matches that checklist; actual smoke execution is the user's responsibility (see External Validation Checklist).
...
- [`services/discord-bot/index.ts`](/home/jared/Nexus-Terminal/services/discord-bot/index.ts) — TypeScript bot entrypoint. Uses `discord.js@14` `Client` with `GatewayIntentBits.Guilds | GuildMessages | MessageContent`. Listens on `messageCreate` only in the `#orchestrator` channel (resolved by name from the configured `DISCORD_GUILD_ID`). For each non-bot message, posts to `${NEXUS_API_URL}/api/agents/service/chat` with `x-agent-service-key` header and body `{ message, discord_user_id: msg.author.id, channel: 'discord', session_id: '<guild>:<channel>:<author>' }`. On 201, polls `GET /api/agents/service/chat?job_id=<id>` every 2s for up to 60 attempts (120s). On `completed` with `result.message`, posts one or more embeds titled `Orchestrator Reply`, color `0x10B981`, and description chunks of the returned message; on `completed` with `result.routed === true`, replies `Your request was routed to a specialist job: <specialistJobId>.`; on `failed`, replies with a plain-text orchestrator failure summary; on timeout, replies `The orchestrator did not finish within 2 minutes. Please try again.`; on submit or poll HTTP errors, replies with a user-safe plain-text error without echoing the upstream response body to Discord. Never calls `/api/agents/admin/*` or any user-facing route.
...
- [`docs/ops/agents-deploy-smoke.md`](/home/jared/Nexus-Terminal/docs/ops/agents-deploy-smoke.md) — the canonical manual launch checklist matching the full External Validation Checklist below. It includes the operator-owned pre-smoke prerequisites, the live smoke walk-through, and the post-smoke closeout items in the same order with the same expected results. Each item is a checkbox plus the exact command or Discord interaction to run; when a checklist item needs deeper env/config detail, the smoke doc points to `agents-launch-validation.md` rather than inventing a separate procedure.
- [`docs/ops/agents-launch-validation.md`](/home/jared/Nexus-Terminal/docs/ops/agents-launch-validation.md) — pre-launch config and secret re-validation checklist (AEV2-510). Documents how to re-verify the lane keys, base URLs, models, `NEXUS_API_URL`, the two service/admin keys, all six webhook URLs, Vercel-to-`services/.env` parity for `AGENT_SERVICE_KEY` and `AGENT_ADMIN_KEY`, and the canonical `npm run typecheck:services` procedure before `docker compose up -d`.
- [`docs/ops/agents-backup-restore.md`](/home/jared/Nexus-Terminal/docs/ops/agents-backup-restore.md) — retroactive Neon backup/restore doc. Documents the exact Neon "branch" backup/export procedure and the exact restore procedure using placeholders where the user must supply branch IDs or notes. The doc also includes a clearly marked operator-owned section for recording tested-restore verification after handoff; that verification is an external launch-readiness item, not a code-side Checkpoint 6 requirement. Lives in `docs/ops/` even though `AGENTIC_EXPANSIONV2.md` §16 originally assigned it to EPIC-2 — Sprint 4 lands it as a launch-readiness artifact since EPIC-2 closed without it.
  ...
  for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt++) {
    ...
    if (body.status === 'completed') {
      if (body.result?.routed) {
        await msg.reply(`Your request was routed to a specialist job: ${body.result.specialistJobId ?? 'unknown'}.`);
        return;
      }
...
The smoke runbook (`agents-deploy-smoke.md`) MUST be a 1:1 mirror of the full External Validation Checklist below — operator-owned pre-smoke prerequisites, the live smoke walk-through, and the after-the-smoke closeout items, with the same checkboxes, same order, and same expected outputs. The user runs that file as the canonical launch checklist after Sprint 4 lands.

`agents-launch-validation.md` is the canonical home for the detailed env/config parity instructions, including `NEXUS_API_URL`, Vercel-to-`services/.env` key parity, and the exact `npm run typecheck:services` procedure. `agents-deploy-smoke.md` may reference that runbook where needed, but it must preserve the External Validation Checklist order verbatim.
...
6. Land the five ops markdown files (Checkpoint 6, including the retroactive backup/restore doc).
...
- [ ] The script is documented canonically in `docs/ops/agents-launch-validation.md` and referenced from `docs/ops/agents-deploy-smoke.md` as a pre-smoke prerequisite (Checkpoint 6; deferred to that checkpoint).
...
**Review focus:** confirm the five runbook files exist, each is 100–200 lines of plain markdown, the smoke runbook mirrors the full External Validation Checklist below 1:1, the launch-validation runbook covers the full env/config parity gate, and no runbook invents repo scripts, internal paths, or code paths the repo does not support.

**Suggested commit:** `docs(aev2): add ops rollback, recovery, smoke, launch-validation, and backup runbooks`

**Check off before commit**

- [x] `docs/ops/agents-rollback.md` covers Vercel rollback, Docker service rollback, and migration 0019 partial-failure recovery.
- [x] `docs/ops/home-server-recovery.md` covers WSL2 reboot, Docker daemon restart, `docker compose up -d`, healthcheck verification, and the post-ISP-outage / post-power-loss sequence.
- [x] `docs/ops/agents-deploy-smoke.md` mirrors the full External Validation Checklist below — same pre-smoke, live smoke, and post-smoke items, same order, same expected results.
- [x] `docs/ops/agents-launch-validation.md` covers the AEV2-510 acceptance gate: re-verifying lane keys, base URLs, models, `NEXUS_API_URL`, `AGENT_ADMIN_KEY`, `AGENT_SERVICE_KEY`, all six webhook URLs, Vercel-to-`services/.env` parity for the two keys, and the canonical `npm run typecheck:services` procedure from Checkpoint 4.
- [x] `docs/ops/agents-backup-restore.md` exists with the retroactive Neon backup/restore steps. Branch IDs remain placeholders for the user to fill if not provided, and the doc includes an operator-owned section for recording tested-restore verification after handoff.
- [x] No runbook invents a repo script, internal path, or code path that does not exist. Standard external ops commands like `docker compose`, `psql`, `curl`, `vercel rollback`, `wsl --shutdown`, `systemctl`, and `sudo reboot` are expected.
- [x] No runbook contains a real secret value.

**Exit criteria**

- [x] AEV2-508 and AEV2-510 acceptance criteria are met on the code/documentation side.
- [x] The smoke runbook is the canonical AEV2-509 artifact — actual smoke execution, Vercel parity confirmation, and tested-restore verification remain the user's responsibility, tracked via the External Validation Checklist below.
...
- [ ] **Record tested-restore verification in `docs/ops/agents-backup-restore.md`.** Use the operator-owned section in that runbook to capture whether a restore drill has been executed, what branch was used, and any verification notes. If a restore drill has not been run yet, note that explicitly and complete it before claiming launch readiness.
...
- [ ] **Discord `#orchestrator` smoke — handle-directly.** Post a plain message ("what's the market doing today?"). Within 60 seconds, the bot should reply with an Orchestrator embed. Verify the embed color is emerald and the title is `Orchestrator Reply`.
- [ ] **Discord `#orchestrator` smoke — routed-to-specialist.** Post `/research AAPL`. Within 30 seconds, the bot should reply with `Your request was routed to a specialist job: <id>.` The Small Cap Trader should pick up the routed job and post a research embed in the `#small-cap-research` channel within ~120 seconds (depending on AskEdgar latency).
