...
- [ ] Focused tests cover breaker open/reset behavior, rate-limit rejection, agent-scoped memory reads, and macro-summary lookup without `macro_summaries`.
...
- [ ] `npm run lint`
- [ ] `npx tsc --noEmit`
- [ ] `npx vitest run __tests__/agent-runtime-limits.test.ts __tests__/agent-memory.test.ts`
