...
### Fix BacktestStatsView owner badge (client/server user-id mismatch)

Status: completed 2026-06-08 (commit `c9b1bf7`); reviewed against spec — READY TO SHIP.

Outcome:
- `GET /api/backtests/[id]` now returns canonical `currentUserId` (`authState.user.id`) in both the normal and `uncat-` branches.
- `useBacktestStats` prefers `serverUserId ?? sessionUserId` (mirrors `use-backtest-manager`); `BacktestStatsView` trusts the hook and dropped its `currentUserId` prop, so the owner-only Edit Notes / Delete dropdown shows on your own stats-view reviews even when session JWT id ≠ DB id.
- `BacktestChartWorkspace` + `BacktestingTab:72` wiring untouched. No schema/migration.

Validation: `npm run lint`, `npx tsc --noEmit`, `npm test` (109 files / 815 tests), `npm run build` all green (build needed a `.next` clean — same stale-cache phantom as the prior review).

### Sheets Massive Wave, Spec 2: CSV import with mapping-review

Status: completed 2026-06-08 (commit `c6630b7`); reviewed against spec — READY TO SHIP.

Outcome:
- Toolbar **Import** button parses a `.csv` client-side (papaparse) and opens an editable mapping modal: auto-detects type, harvests select options, maps Date/Ticker onto locked columns, drops Reports/Charts/Sim Trade, maps Float/$ Vol/Share Vol to the Spec 1 Massive types.
- Pure `lib/sheets/import.ts` (`detectImportColumns`/`buildImportPayload`/`coerceImportNumber`) handles inference, `$`/comma coercion, checkbox bools, and ticker-less row skips; unit-tested.
- New `POST /api/sheets/import` creates one multi-date sheet (`sheetDate: null`) + owner membership + bulk rows in one transaction; `useSheets.importSheet` makes it active immediately. No schema/migration change.

Validation: `npm run lint`, `npx tsc --noEmit`, `npm test` (109 files / 814 tests), `npm run build` all green (build needed a `.next` clean — first run hit a stale-cache phantom on an unrelated route). LOW note: imported sheet's owner label is blank until next list refresh (self-owned, cosmetic).
